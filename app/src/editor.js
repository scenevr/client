...
Editor.prototype.getDropPosition = function () {
  ...
  var direction = this.client.controls.getDirection(new THREE.Vector3());
...
Editor.prototype.onAddCube = function () {
  var el = $('<box />');
  el.attr('position', this.getDropPosition().toArray().join(' ')).attr('style', 'color : #f70');
...
Editor.prototype.showMenu = function () {
  ...
  var div = $('<div />');
  var ul = $('<ul />').addClass('menu').appendTo(div);
...
Editor.prototype.startDragging = function () {
  var obj = this.getSelectedObject();
...
Editor.prototype.onUpdate = function (controls) {
  ...
  var position = snapToGrid(player.clone().add(direction.multiplyScalar(2)));
